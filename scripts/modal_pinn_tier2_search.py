"""
modal_pinn_tier2_search.py
----------------------------
Tier 2/3 PINN search on Modal Cloud GPUs (NVIDIA A10G), building on the honest
Tier 0/1 protocol already established in scripts/modal_pinn_tier1_search.py
(nested proxy-split selection, mini-batch, no pos_loss, log-space deep ensemble).
Tier 1's honest result was ensemble raw R^2 = 0.7316 on Split C -- still below
the GridInterp(raw) baseline of 0.8415. This script implements PLAN.md's two
highest-confidence remaining levers to close that gap:

  Tier 3 (preferred): RESIDUAL/MULTIPLICATIVE LEARNING ON TOP OF GridInterp.
    Instead of predicting log(CHF) directly, the network predicts a correction
    c(P,G,X) such that CHF_pred = GridInterp_base(P,G,X) * exp(c(P,G,X)). Since
    GridInterp alone already reaches R^2=0.8415, the network only has to learn
    the (much smaller) residual structure GridInterp gets wrong, instead of the
    full CHF response from scratch. The base values (grid_base_proxy / grid_base_final)
    are precomputed leakage-free via leave-one-pressure-out in
    scripts/prepare_gridinterp_bases.py -- see that file's docstring for why a
    naive same-fit base would give a trivial near-zero residual target.
    The physics penalties (monotonicity-in-X, Zuber pressure-trend sign) are
    applied to the raw correction network output at collocation points, exactly
    as in Tier 1 -- i.e. they still shape the learned correction function, not a
    non-differentiable GridInterp call.

  Tier 3: SiLU vs Tanh activation. Tanh saturates outside the training range, so
    an extrapolated correction can flatten instead of continuing a trend; SiLU's
    near-linear tail stays smooth (still differentiable for the physics penalties)
    without saturating.

Ablation grid (fixed arch=[16,8], the clear Tier-1 proxy winner -- see
tier1_proxy_search_results.csv top rows): target_mode x activation x
(lam_mono, lam_zuber) in {(0,0), (0.3,0), (0,0.1), (0.3,0.1)}. This directly
produces the "no-physics -> +monotonicity -> +pressure-trend -> both" ablation
table PLAN.md's "Realistic target & thesis framing" section asks for, crossed
with the two new Tier-3 axes, still selected purely on PROXY validation
(P 15000-16000 kPa) -- the real Split C test set is touched exactly once, for
the single winning config, exactly as in Tier 1.
...
app = modal.App("chf-pinn-tier2-search")


def _build_model(hidden_layers, in_dim=3, activation="tanh"):
...
def _train_one(Xtr, ytr, base_tr, Xval, yval, base_val, hidden_layers, activation,
    ...
    """target_mode: 'direct' (predict log(CHF)) or 'residual' (predict
    log(CHF) - log(base), reconstructed as base * exp(pred) at inference).
    base_tr/base_val are the leakage-free GridInterp base columns (ignored
    entirely when target_mode == 'direct')."""
    import torch
    import torch.nn as nn
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, n, batch_size):
            ...
            # Physics penalties act on the network's raw output (the correction term
            # in residual mode, or the full log-CHF prediction in direct mode) --
            # keeps the graph fully differentiable; see module docstring.
...
def proxy_search_worker(config_json: str, seeds: list):
    ...
    base_tr = proxy_train["grid_base_proxy"].values
    base_val = proxy_val["grid_base_proxy"].values
    ...
    for seed in seeds:
        _, _, _, _, pred_val, _, _ = _train_one(
    ...
    return {
        "target_mode": config["target_mode"], "activation": config["activation"],
...
# ============================================================================
# 2. FINAL EVALUATION WORKER (real Split C, touched once per config, ensemble seeds)
# ============================================================================
@app.function(image=image, gpu="A10G", timeout=900)
...
def main():
    print("=" * 80)
    print("TIER 2/3 PINN SEARCH — residual-on-GridInterp + SiLU ablation, honest metrics")
    print("=" * 80)

    arch = [16, 8]  # fixed: clear Tier-1 proxy winner, see tier1_proxy_search_results.csv
    lr = 1e-3
    n_collocation = 512
    seeds = [0, 1, 2, 42, 99]

    physics_pairs = [
        (0.0, 0.0),   # no-physics baseline
        (0.3, 0.0),   # +monotonicity
        (0.0, 0.1),   # +pressure-trend (Zuber sign)
        (0.3, 0.1),   # both
    ]
    target_modes = ["direct", "residual"]
    activations = ["tanh", "silu"]

    combos = [
        {"hidden_layers": arch, "target_mode": tm, "activation": act,
         "lam_mono": m, "lam_zuber": z, "lr": lr, "n_collocation": n_collocation}
        for tm in target_modes for act in activations for (m, z) in physics_pairs
    ]
    ...
    print("Axes: target_mode {direct, residual-on-GridInterp} x activation {tanh, silu} x "
          "(lam_mono, lam_zuber) in {(0,0),(0.3,0),(0,0.1),(0.3,0.1)}, arch fixed at [16,8].")
    ...
    proxy_df.to_csv(results_dir / "tier2_proxy_search_results.csv", index=False)

    print("\n" + "=" * 100)
    print("ALL CONFIGS BY PROXY VALIDATION (log R^2) — selection was NOT based on real Split C")
    print("=" * 100)
    ...
    best_config = {
        "hidden_layers": arch,
        "target_mode": best_row["target_mode"], "activation": best_row["activation"],
        "lam_mono": float(best_row["lam_mono"]), "lam_zuber": float(best_row["lam_zuber"]),
        "lr": lr, "n_collocation": n_collocation,
        "epochs": 1000, "patience": 50, "batch_size": 2048,
    }
    print(f"\nWinning config (by proxy val): {best_config}")

    # ---- Ablation table: winning (target_mode, activation) combo across all 4 physics pairs ----
    print("\nEvaluating full physics ablation (no-physics / +mono / +zuber / +both) for the "
          f"winning (target_mode={best_config['target_mode']}, activation={best_config['activation']}) "
          "combo on the real Split C test set, 10 seeds each, touched once per row.")

    ablation_rows = []
    ensemble_by_physics = {}
    final_seeds = list(range(10))
    for (m, z) in physics_pairs:
        cfg = dict(best_config, lam_mono=m, lam_zuber=z)
        ...
        print(f"  lam_mono={m} lam_zuber={z}: single-seed R2={final_df.raw_r2.mean():.4f}"
              f"+/-{final_df.raw_r2.std():.4f}  ensemble R2={ens_raw_r2:.4f}  ({dt:.0f}s)")

    ablation_df = pd.DataFrame(ablation_rows)
    ablation_df.to_csv(results_dir / "tier2_ablation_table.csv", index=False)

    print("\n" + "=" * 80)
    print("ABLATION TABLE (winning target_mode/activation, real Split C, 10-seed ensemble)")
    print("=" * 80)
    print(ablation_df.to_string(index=False))
    print(f"\nReference — GridInterp (deterministic) baseline on Split C: raw R^2 = 0.8415")
    print(f"Reference — Tier 1 honest ensemble (direct target, tanh): raw R^2 = 0.7316")

    # ---- Per-pressure-level breakdown for the best ablation row's ensemble ----
    best_ablation_idx = ablation_df["ensemble_raw_r2"].idxmax()
    best_physics = (float(ablation_df.loc[best_ablation_idx, "lam_mono"]),
                     float(ablation_df.loc[best_ablation_idx, "lam_zuber"]))
    y_true, p_col, ensemble_pred = ensemble_by_physics[best_physics]

    print("\n" + "=" * 80)
    print(f"PER-PRESSURE-LEVEL BREAKDOWN (best ablation row: lam_mono={best_physics[0]}, "
          f"lam_zuber={best_physics[1]})")
    print("=" * 80)
    breakdown_rows = []
    for p_level in sorted(np.unique(p_col)):
        mask = p_col == p_level
        r2, log_r2, mape = _metrics(y_true[mask], ensemble_pred[mask])
        breakdown_rows.append({"P_kPa": p_level, "n": int(mask.sum()), "raw_r2": r2,
                                "log_r2": log_r2, "mape": mape})
        print(f"  P={p_level:>7.0f} kPa  n={int(mask.sum()):>4d}  raw_R2={r2:>7.4f}  "
              f"log_R2={log_r2:>7.4f}  MAPE={mape:>6.2f}%")
    breakdown_df = pd.DataFrame(breakdown_rows)
    breakdown_df.to_csv(results_dir / "tier2_per_pressure_breakdown.csv", index=False)
    ...
    summary = {
        ...
        "best_ensemble_raw_r2": float(ablation_df.loc[best_ablation_idx, "ensemble_raw_r2"]),
        "best_ensemble_log_r2": float(ablation_df.loc[best_ablation_idx, "ensemble_log_r2"]),
        "best_ensemble_mape": float(ablation_df.loc[best_ablation_idx, "ensemble_mape"]),
        "gridinterp_baseline_raw_r2": 0.8415,
        "tier1_ensemble_raw_r2": 0.7316,
        "beats_gridinterp": bool(ablation_df.loc[best_ablation_idx, "ensemble_raw_r2"] > 0.8415),
        "proxy_search_seconds": t_proxy,
    }
    with open(results_dir / "tier2_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\nSaved: results/pinn/tier2_proxy_search_results.csv, tier2_ablation_table.csv, "
          f"tier2_per_pressure_breakdown.csv, tier2_summary.json")
